...
// Pas de localStorage dans les artifacts -> état en mémoire uniquement (persistant pendant la session)
let memoryStore = {};
...
export default function App() {
  ...
  // Charger l'état mémoire au montage
  useEffect(() => {
      ...
      try {
        ...
        setPredictions((prev) => {
          ...
          for (const [matchId, byPlayer] of Object.entries(data.predictions)) {
            merged[matchId] = { ...(merged[matchId] || {}), ...byPlayer };
          }
          return merged;
        });
...
function MatchCard({ match, myPred, othersPreds, player, onSave }) {
  ...
  const locked = played; // verrou permanent une fois le match joué
    ...
    <div style={{ ...styles.card, ...(locked ? styles.cardLocked : {}) }}>
      ...
      {canPredict && (
        <button
          ...
          onClick={handleSave}
        >
          {hasSavedPred ? "Modifier mon pronostic" : "Valider mon pronostic"}
        </button>
      )}
  ...
  if (locked) {
    return (
      <div style={styles.scoreBoard}>
        <span style={styles.digit}>{score1}</span>
        <span style={styles.scoreDash}>–</span>
        <span style={styles.digit}>{score2}</span>
      </div>
    );
  }
  if (!canPredict) {
  ...
  return (
    <div style={styles.content}>
      ...
      <div style={styles.dataNote}>
        Les données sont stockées uniquement sur cet appareil, pour cette session. Pensez à
        exporter régulièrement vos pronostics afin de ne pas les perdre.
      </div>
    </div>
  );
}
...
const styles = {
  ...
  gateScreen: {
    minHeight: "100vh",
    background: `radial-gradient(circle at 50% 20%, #1a2740 0%, ${COLORS.bg} 70%)`,
    display: "flex",
    alignItems: "center",
